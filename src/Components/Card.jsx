...
const Card = ({ img, title, description, id }) => {
    return (
        <Link to={`/${id}`} className="relative w-[327px] h-[343px] group">
            ...
            <div className="relative w-full h-full bg-[#efefef] clip-path-custom transition-transform transform group-hover:-translate-y-2 group-hover:shadow-lg">
                <div className="p-4 text-gray-800">
                    <div className='rounded-2xl w-full h-auto aspect-[3/2] overflow-hidden'>
                        <img className='transition-transform transform' src={img} alt={title} />
                    </div>
                    <h2 className="text-xl font-bold mb-2 transition-transform transform py-1">{title}</h2>
                    <p className="transition-opacity opacity-60 group-hover:opacity-100 max-w-[240px] line-clamp-2">{description}</p>
                </div>
            </div>
